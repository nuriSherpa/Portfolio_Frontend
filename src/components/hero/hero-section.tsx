...
export function HeroSection({ hero, stories, serverStats }: HeroSectionProps) {
  console.log('[HeroSection] serverStats:', serverStats); // ← verify prop arrives here
  ...
  const handleProfileClick = () => {
    if (stories.length > 0) setShowStoryModal(true);
    else setShowNoStoriesModal(true);
  };
  ...
  const allStoriesViewed = stories.length > 0 && stories.every((s) => viewedStories.has(s.id));
    ...
    <div className="w-full flex justify-center">
      <div className="w-[90%] sm:w-[85%] md:w-[80%] lg:w-[80%] max-w-6xl px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
        <div className="flex flex-col items-center">
          ...
          <div
            className="relative w-48 h-48 lg:w-56 lg:h-56 transition-transform hover:scale-105 mb-6 cursor-pointer"
          ...
          <div className="mb-8">
            <HeroStats serverStats={serverStats} />
          </div>
      ...
      {showNoStoriesModal && stories.length === 0 && (
        <NoStoriesModal onClose={() => setShowNoStoriesModal(false)} />
      )}
    </div>
  );
}
